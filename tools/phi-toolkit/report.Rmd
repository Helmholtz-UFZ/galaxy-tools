...
## ------------------------------------------------------------------------
## Helper to write a data.frame to a TSV file in <outdir>/<tool>/<name>.tsv
## Used for both per‑genome tables (sub‑folder = tool name, file = genome) and
## summary tables to a file named by the tool.
## ------------------------------------------------------------------------
write_tsv_table <- function(df, tool, name) {
    dir_path <- file.path(params$outdir, tool)
    if (!fs::dir_exists(dir_path)) fs::dir_create(dir_path)
    readr::write_tsv(df, file.path(dir_path, paste0(name, ".tsv")))
}
```
...
# Write overview summary TSV (plain text, no HTML)
clean_table_data <- table_data %>%
    mutate(`GTDB Taxonomy` = gsub("<[^>]+>", "", `GTDB Taxonomy`),
           Virus_Count = gsub("<[^>]+>", "", Virus_Count),
           Quality_Score = gsub("<[^>]+>", "", Quality_Score),
           Genome_Quality = gsub("<[^>]+>", "", Genome_Quality),
           Completeness = gsub("<[^>]+>", "", Completeness),
           Contamination = gsub("<[^>]+>", "", Contamination))
write_tsv_table(clean_table_data, "Overview", "overview")
    ...
    tryCatch(
        {
            ...
            # Write GTDB‑Tk taxonomy per‑genome TSV
            gtdb_tbl <- data_gtdbtk_host %>%
                dplyr::filter(user_genome == sample) %>%
                dplyr::select(classification)
            write_tsv_table(gtdb_tbl, "GTDB_Tk", sample)
            ...
            # Write detailed CheckM2 summary TSV for this sample
            checkm_summary_tbl <- data_checkm_host %>%
                dplyr::filter(`name` == sample) %>%
                janitor::clean_names() %>%
                dplyr::select(total_contigs, contig_n50, completeness, contamination)
            write_tsv_table(checkm_summary_tbl, "CheckM2_Summary", sample)
            ...
            if (!is.null(defense_finder_data) && nrow(defense_finder_data) > 0) {
                ...
                # Write filtered Defense‑Finder TSV (same columns as markdown)
                defense_finder_tbl <- defense_finder_data %>%
                    dplyr::select(sys_id, type, subtype, sys_beg, sys_end, protein_in_syst, genes_count, name_of_profiles_in_sys)
                write_tsv_table(defense_finder_tbl, "DefenseFinder", sample)
            } else {
            ...
            # Write Genomad + CheckV summary per‑genome TSV
            genomad_checkv_tbl <- genomad_summary %>%
                dplyr::select(seq_name, taxonomy, topology, coordinates, length) %>%
                dplyr::left_join(
                    checkv_data %>% dplyr::select(contig_id, gene_count, viral_genes, checkv_quality, miuvig_quality),
                    by = c("seq_name" = "contig_id")
                )
            write_tsv_table(genomad_checkv_tbl, "Genomad_CheckV", sample)
...
If multiple contigs are present in the genome, each will be shown with a distinct label (e.g., **c1**, **c2**, etc.).\n\n')
            ...
            if (nrow(abricate_data) > 0) {
                ...
                abricate_data %>%
                    ...
                    cat()
                cat("\n\n")
                # Write Abricate virulence‑genes per‑genome TSV
                abricate_tbl <- abricate_data %>% dplyr::select(-number_file)
                write_tsv_table(abricate_tbl, "Abricate", sample)
            }
            ...
            if (nrow(iphop_data) > 0) {
                ...
                iphop_data %>%
                    ...
                    cat()
                cat("\n\n")
                # Write iPHOP host‑prediction per‑genome TSV
                iphop_tbl <- iphop_data
                write_tsv_table(iphop_tbl, "iPHOP", sample)
            }
            ...
            if (nrow(vibrant_data) > 0) {
                ...
                vibrant_data %>%
                    ...
                    cat()
                cat("\n\n")
                # Write VIBRANT AMGs per‑genome TSV
                vibrant_tbl <- vibrant_data
                write_tsv_table(vibrant_tbl, "VIBRANT", sample)
            }
...
process_phage <- function(virus, genomad_summary, genomad_annotation, checkv_data, host_genome_size) {
    ...
    # Write per‑phage gene‑annotation TSV (filename includes sample and virus ID)
    phage_ann_tbl <- current_annotations %>%
        dplyr::select(gene, length, marker, annotation_accessions, annotation_description)
    write_tsv_table(phage_ann_tbl, "GeNomad", str_replace_all(virus$seq_name, "[^A-Za-z0-9.-]", "_"))
}
