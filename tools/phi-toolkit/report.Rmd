...
# Remove any extensions from names in data gtdbtk and checkm2
data_gtdbtk_host <- data_gtdbtk_host %>%
  dplyr::mutate(user_genome = stringr::str_remove(user_genome, "\\.[^.]+$"))
...
checkm_host_data <- data_checkm_host %>% janitor::clean_names() %>%
  dplyr::select(name, completeness, contamination, 
...
table_data <- data %>%
  ...
  dplyr::mutate(
    quality_data = purrr::pmap(list(as.numeric(completeness),
...
kableExtra::kbl(table_data, escape = FALSE,
  ...
  kableExtra::add_header_above(c(" " = 4, "Host Genome Quality" = 2, "CheckM Metrics" = 2,
                     "Statistics" = 2)) %>%
...
plot_genome_ideogram <- function(genome_current, proviruses_gr_features) {
  ...
  for (i in 1:length(proviruses_gr_features)) {
    ...
    circlize::circos.genomicRect(seqnames = name,
                       region,
                       ytop = .5,
                       ybottom = 0,
                       track.index = 1,
  ...
  circlize::circos.labels(sectors = as.character(seqnames(proviruses_gr_features)),
...
process_sample <- function(sample, combined_unique, host_genomes_paths, genome_data) {
  ...
  tryCatch({
    ...
    data_gtdbtk_host %>% dplyr::filter(user_genome == sample) %>%
    ...
    checkm_summary %>% janitor::clean_names() %>%
    ...
    if (length(GenomeInfoDb::seqlevels(host_genome_ideogram)) == 1) {
      host_genome_size <- sum(BiocGenerics::width(host_genome_ideogram))
    } else {
      ...
      host_genome_size <- sum(BiocGenerics::width(filtered_host_genome))
    }
...
If multiple contigs are present in the genome, each will be shown with a distinct label (e.g., **c1**, **c2**, etc.).\n\n')
    ...
    if (nrow(abricate_data) > 0) {
      ...
      abricate_data %>% dplyr::select(-number_file) %>%
      kableExtra::kbl() %>%
      kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
      kableExtra::kable_paper("striped", full_width = TRUE) %>%
      kableExtra::scroll_box(width = "100%", height = "100%") %>% cat()
      cat("\n\n")
    }
...
plot_phage_circos <- function(virus, genomad_summary, current_annotations, virus_length, host_genome_size, provirus_start, provirus_end, checkv_data) {
  tryCatch({
    ...
    circlize::circos.track(factors = "Zoom", ylim = c(0, 1), track.height = 0.15,
                 panel.fun = function(x, y) {
                   ...
                   circlize::circos.axis(h = "top", major.at = axis_positions,
                   ...
                   for (i in 1:nrow(current_annotations)) {
                     ...
                     circlize::circos.arrow(arrow_start, arrow_end, y1 = 0, y2 = 1,
    ...
    circlize::circos.track(factors = "Main", ylim = c(0, 1), track.height = 0.1,
                 panel.fun = function(x, y) {
                   ...
                   for (i in 1:nrow(contig_viruses)) {
                     ...
                     circlize::circos.rect(xleft = virus_start_percent, ybottom = 0,
    ...
    circlize::circos.text(x = center_x, y = -0.2, labels = taxonomy,
