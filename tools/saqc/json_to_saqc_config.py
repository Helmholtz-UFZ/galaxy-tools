...
print("DEBUG: Running json_to_saqc_config.py VERSION_JUNE_2_2025_V5", file=sys.stderr) # Deine Versionsinfo
...
for r_method_set in params_from_galaxy.get("methods_repeat", []):
    ...
    field_str_for_error = "unknown_field_in_repeat" # Wird unten genauer gesetzt
    try:
        ...
        if "field" in params_to_process:
            ...
            field_repeat_data = params_to_process.pop("field_repeat") # Wichtig: pop, damit es nicht als Argument weitergegeben wird
            if isinstance(field_repeat_data, list) and len(field_repeat_data) > 0:
                first_field_item = field_repeat_data[0] # Nimm das erste Element des Repeats
        ...
        if raw_field_val is None or str(raw_field_val).strip() == "": # Wenn kein Feld gefunden oder Feld ist leer
            ...
            # Für den Fall, dass raw_field_val eine Liste sein könnte (wenn du mehrere Felder aus field_repeat aggregieren würdest)
        ...
        # *** ENDE DER MODIFIKATION FÜR FIELD EXTRACTION ***

        saqc_args_dict = {}

        for param_key, param_value_json in params_to_process.items():
            if param_key.endswith("_select_type"): # Überspringe interne Galaxy-Selektoren
                continue
            ...
            # Beachte: Die spezielle Behandlung von "" für bestimmte Parameter ist hier beibehalten.
            # Wenn ein "" für einen normalen Textparameter kommt, der nicht in der Sonderliste ist, wird er als "" übergeben.
        ...
        for k_saqc, v_saqc_raw in sorted(saqc_args_dict.items()):
            ...
            if v_saqc_raw is None:
                ...
                if not v_saqc_raw and k_saqc not in ["xscope", "yscope", "max_gap", "min_periods", "min_residuals", "min_offset"]: # Wenn "" und nicht Spezialfall, als "" String
                    # Das ursprüngliche Skript hat hier implizit leere Strings zu None gemacht, wenn k_saqc *nicht* in der Liste war
                    # Dies ist korrigiert, um leere Strings als "" zu behandeln, es sei denn, sie sind explizit None (siehe oben)
                    # oder für die Spezialparameter. Für die meisten Textparameter sollte "" -> '""' sein.
                    # Die ursprüngliche Logik für "if not v_saqc_raw: v_str_repr = "None"" war etwas zu aggressiv.
                    # Wir wollen nur None, wenn es explizit None ist oder "" für die Spezialparameter.
                    # Wenn ein anderer Parameter einen leeren String hat, sollte er als '""' enden.
                    val_lower = v_saqc_raw.lower() # Dieser Block wird bei leerem String nicht erreicht, wenn oben "" -> None war
